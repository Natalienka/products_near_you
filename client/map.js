(function() {
  var HOST_NAME = "http://127.0.0.1:5000";
  ...
  */
  ...
  var ProductMap = function(center, searchRadius) {
    ...
    var getProductLayer = function(data) {
      var features = data.map(function(shop) {
        var products = shop.products;
        var highestPopularity = products[0].popularity;
        var color = hslToHex(0.2 * (1 - highestPopularity), 0.8, 0.5);

        var description = products.reduce(function(acc, product) {
            return acc + product.title + " (" + product.quantity + " available)<br>"
        }, "");

        var title = shop.name + " - " + Math.round(shop.distance) + "m";
    ...
    this.plot = function(data) {
      ...
      this.productLayer = getProductLayer(data);
    ...
    this.search = function(cb) {  
      ...
      $.ajax({
        ...
        success    : function(response){
          cb(null, response.shops);
        },
        error: function(xhr, status, error) {
          cb("A problem occurred while searching for products.", null);
        }
      });
    };
  };
  ...
  var init = function() {
    ...
    controls.on('search', function() {
      searcher.search(function(err, result) {
